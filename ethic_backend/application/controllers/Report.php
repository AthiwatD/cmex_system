...
class Report extends MY_Controller {
    ...
    function reports(){
        ...
        $this->data['evaluations'] = $this->EvaluaterPerson->getEvaluates();
	...
	function reportPointPersons($evaluation_id){
		$this->data['error'] = $this->db->error(); 
        $this->data['evaluations'] = $this->EvaluaterPerson->getEvaluates();
	...
	/////////////////////////////////////////////////////////////////
	//
	//		รายงานประเมิณโดยตัวเอง
	//
	/////////////////////////////////////////////////////////////////
    function report_self($evaluation_id){
        $init_height = 200;
        $row_height = 50;
        $this->data['error'] = $this->db->error(); 
        ...
        foreach($report_by_center as $index => $rpt_cen){
            ...
            array_push($report_by_center_data, round(($rpt_cen->average_point*100)/$report_form[0]->max_point,2));
			...
			array_push($report_by_center_max_point, round(100,2));
        }
        ...
        foreach($report_category as $index => $rpt_cat){
            ...
            array_push($report_category_data, round(($rpt_cat->average_point*100)/$rpt_cat->max_point,2));
            // array_push($report_category_max_point, round($rpt_cat->max_point,2));
			array_push($report_category_max_point, round(100,2));
        }
        ...
        $this->breadcrumb->add('หน้าหลัก', base_url() .'Home');      
        $this->breadcrumb->add('รายการแบบประเมิน',   base_url().'Report/reports');  
        $this->breadcrumb->add('รายงานคะแนนประเมิณตนเอง',   base_url().'Report/report_self/' . $evaluation_id);      
        $this->data['breadcrumb'] = $this->breadcrumb->output();
	...
	/////////////////////////////////////////////////////////////////
	//
	//		รายงานประเมิณโดยหัวหน้า
	//
	/////////////////////////////////////////////////////////////////
	function report_head($evaluation_id){
        $init_height = 200;
        $row_height = 50;
        $this->data['error'] = $this->db->error(); 
        $this->data['evaluation_id'] = $evaluation_id;
		...
		// คะแนนเฉลี่ย by Center
		$report_by_center = $this->Report->report_head_by_center($evaluation_id);
        $this->data['report_by_center'] = $report_by_center;
        $report_by_center_label = array();
        $report_by_center_data = array();
        $report_by_center_max_point = array();
        ...
        $report_category = $this->Report->report_head_category($evaluation_id);
        $this->data['report_category'] = $report_category;
        $report_category_label = array();
        $report_category_data = array();
        $report_category_max_point = array();
        ...
        $report_question_group = $this->Report->report_head_question_group($evaluation_id);
        $this->data['report_question_group'] = $report_question_group;
        $report_question_group_label = array();
        $report_question_group_data = array();
        foreach($report_question_group as $index => $rpt_qg){
        ...
        $report_question = $this->Report->report_head_question($evaluation_id);
        $this->data['report_question'] = $report_question;
        $report_question_label = array();
        $report_question_data = array();
        $question_chart_height = array();
        ...
        $this->data['report_suggestion'] = $this->Report->report_head_suggestion($evaluation_id);

        $this->breadcrumb->add('หน้าหลัก', base_url() .'Home');      
        $this->breadcrumb->add('รายการแบบประเมิน',   base_url().'Report/reports');  
        $this->breadcrumb->add('รายงานคะแนนจากหัวหน้า',   base_url().'Report/report_head/' . $evaluation_id);      
        $this->data['breadcrumb'] = $this->breadcrumb->output();
	...
	//
	/////////////////////////////////////////////////////////////////
	function report_average($evaluation_id){
        $init_height = 200;
        $row_height = 50;
        $this->data['error'] = $this->db->error(); 
        $this->data['evaluation_id'] = $evaluation_id;
		...
		// คะแนนเฉลี่ย by Center
		$report_by_center = $this->Report->report_by_center($evaluation_id);
		$report_head_by_center = $this->Report->report_head_by_center($evaluation_id);
        ...
        foreach($report_by_center as $index => $rpt_cen){
            // array_push($report_by_center_label, $rpt_cen->center_abbre . "." . $rpt_cen->center_name);
			array_push($report_by_center_label, $rpt_cen->center_name);
            array_push($report_by_center_data, round(((($report_by_center[$index]->average_point + $report_head_by_center[$index]->average_point)/2)*100)/$report_form[0]->max_point,2));
            // array_push($report_by_center_max_point, round($rpt_cen->max_point,2));
        ...
        $report_category = $this->Report->report_category($evaluation_id);
		$report_head_category = $this->Report->report_head_category($evaluation_id);
        $this->data['report_category'] = $report_category;
        $report_category_label = array();
        $report_category_data = array(); 
        $report_category_max_point = array();
        foreach($report_category as $index => $rpt_cat){
            array_push($report_category_label, $rpt_cat->category_number . "." . $rpt_cat->category_name);
            array_push($report_category_data, round(((($report_category[$index]->average_point + $report_head_category[$index]->average_point)/2)*100)/$rpt_cat->max_point,2));
            // array_push($report_category_max_point, round($rpt_cat->max_point,2));
			array_push($report_category_max_point, round(100,2));
        }
        $this->data['report_category_label'] = $report_category_label;
		...
		$report_question_group = $this->Report->report_question_group($evaluation_id);
        $report_head_question_group = $this->Report->report_head_question_group($evaluation_id);
        $this->data['report_question_group'] = $report_question_group;
        $report_question_group_label = array();
        $report_question_group_data = array();
        foreach($report_question_group as $index => $rpt_qg){
            array_push($report_question_group_label, $rpt_qg->question_group_number . "." . $rpt_qg->question_group_name);
			$avg = ($report_question_group[$index]->average_point + $report_head_question_group[$index]->average_point)/2;
            array_push($report_question_group_data, round(($avg*100)/$rpt_qg->max_choice_point,2));
        }
        $this->data['report_question_group_label'] = $report_question_group_label;
        $this->data['report_question_group_data'] = $report_question_group_data;
        $this->data['question_group_chart_height'] = $init_height + (sizeof($report_question_group_data) * $row_height);
        ...
        $report_question = $this->Report->report_question($evaluation_id);
		$report_head_question = $this->Report->report_head_question($evaluation_id);
        ...
        foreach($report_question as $index => $rpt_q){
            ...
            array_push($report_question_label[$i], $rpt_q->question_group_number . "." . $rpt_q->question_number . " " . $rpt_q->question_name);
			$avg = ($report_question[$index]->average_point + $report_head_question[$index]->average_point)/2;
            array_push($report_question_data[$i], round(($avg*100)/$rpt_q->max_choice_point,2));
        }
        $question_chart_height[$i]  = $init_height + (sizeof($report_question_data[$i]) * $row_height);
        $this->data['report_question_label'] = $report_question_label;
        $this->data['report_question_data'] = $report_question_data;
        ...
        $this->data['report_suggestion'] = $this->Report->report_suggestion($evaluation_id);
		$this->data['report_head_suggestion'] = $this->Report->report_head_suggestion($evaluation_id);

        $this->breadcrumb->add('หน้าหลัก', base_url() .'Home');      
        $this->breadcrumb->add('รายการแบบประเมิน',   base_url().'Report/reports');  
        $this->breadcrumb->add('รายงานคะแนนเฉลี่ย',   base_url().'Report/report_average/' . $evaluation_id);      
        $this->data['breadcrumb'] = $this->breadcrumb->output();
