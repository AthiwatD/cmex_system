<script>
    ...
    function update_evaluater_persons(){
        if(confirm("คุณต้องการ บันทึก?")){
			var evaluater_id = document.getElementById("evaluater_id").value;
			if(evaluater_id != ""){
				update_item_arr();
				...
				ourObj.evaluater_id = document.getElementById("evaluater_id").value;
				ourObj.persons = persons_arr;
				var points = JSON.stringify(ourObj);
				// points = valid_json_char(points);
				// console.log(points);
				$.ajax({
					url: "<?php echo base_url(); ?>EvaluaterPerson/updateEvaluaterPersonsDo",
					method: 'POST',
					headers: {
