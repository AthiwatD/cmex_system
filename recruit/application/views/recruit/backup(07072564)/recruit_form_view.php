    ...
    <div class="row">
      ...
      <div class="col-md-12">
        ...
        <?php
          ...
          if($method == "update"){ $update=1; echo '<script> var check_update_hd ="'.$update.'";</script>'; ?><?php
          }else{ $update=0; echo '<script> var check_update_hd ="'.$update.'";</script>'; }
        ?>

				<form id="form_publish" role="form" enctype="multipart/form-data" class="needs-validation" method="POST" <?php
					 if($method == "add") echo " action='" . base_url() . "Recruit/addRecruitProcess/$session_username' ";
				    ...
				    <div class="form-group">
				        <div class="row">
		                    ...
		                    foreach($locations as $location){
		                    	if($method=="update"){?>
		                    			<option value='<?php echo $location->ward_code; ?>' <?php if($location->ward_code==$recruitHd->ward_code) echo "selected"; ?>><?php echo $location->ward_name1; ?></option>"; <?php
		                    	}else echo "<option value='$location->ward_code'>$location->ward_name1</option>";
		                    }
				    ...
				    <div class="form-group">
				        <div class="row">
		                    ...
		                    foreach($positions as $position){
		                    	if($method=="update"){ ?>
		                    		<option value='<?php echo $position->position_id; ?>' <?php if($position->position_id==$recruitHd->position_id) echo "selected"; ?>><?php echo $position->position_name; ?></option>"; <?php
		                    	}else echo "<option value='$position->position_id'>$position->position_name</option>";
		                    }
				        ...
				        <div class="row mb-3">
				            ...
				            <div class="col-sm-9">
				                <select id="position_type_id" name="position_type_id" class="form-control " style="max-width:100%;" required>
				                    <?php
				                    //[Athiwat][01/07/2564][fixed for concept use query from model]
				                    $positiontypes=$this->RecruitModel->getPositionTypes();
				                    foreach($positiontypes as $positiontype){
				                    	if($method=="update"){?>
				                    		<option value='<?php echo $positiontype->position_type_id; ?>' <?php if($positiontype->position_type_id==$recruitHd->position_type_id) echo "selected"; ?>><?php echo $positiontype->position_type_name; ?></option>"; <?php
				                    	}else if($method=="add"){
				                    		echo "<option value='$positiontype->position_type_id'>$positiontype->position_type_name</option>";
				                    	}
				                    }
              ...
              <table id="table_view_publish_dt" class="table table-flush table-striped table-bordered table-hover col-12">
                  <thead class="thead-light">
                      <tr>
                          ...
                          <?php
                          	if($method=="update"){ ?>
                          		<th style="width:110px;">จัดการ</th><?php
                          	}else{ ?>
                          		<th style="width:110px;">เอกสาร</th><?php
                          	}
                          ?>
                      </tr>
                  </thead>
                  <tbody id="tbody_view_publish_dt"><?php
                  		for($i=0;$i<count(array_filter($recruitDt));$i++){ 
                      		...
                      		<td><a href="<?php echo $patchfile; ?>" class="" target="_blank"><?php echo $recruitFile[$i]->file_name; ?></a></td>
