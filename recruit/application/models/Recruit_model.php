<?php
	...
	class Recruit_model extends CI_model{
		...
		function deleteRecruitDt($hdid,$dtid,$formArrayDT){
			// $sql_update="deleted=1 ,modify_date=$arrayData[modify_date] ,modify_by=$arrayData[modify_by]";
			// $last=end($arrayData);
			// $sql_update="deleted=1 ,modify_date='$arrayData[modify_date]' ,modify_by='$last'";
			
			// $sql="UPDATE rc_publish_dt set deleted=1 ,modify_date='$datetime' ,modify_by='$wardcode' where publish_hd_id=$hd_id and publish_dt_id=$dt_id";
			// $this->db->update($sql);
		...
		function getRecruitHds(){
			$sql="SELECT * FROM rc_publish_hd WHERE deleted=0 ORDER BY start_date DESC";
			$result = $this->db->query($sql)->result();
			return $result;
		}

		function getRecruitDts(){
			$sql="SELECT * FROM rc_publish_dt WHERE deleted=0 ORDER BY create_date DESC";
			$result=$this->db->query($sql)->result();
			return $result;
		}
