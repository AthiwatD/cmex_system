<?php
	class Repass_model extends CI_model{
		...
		public function getoldPassword($num_ot){
			$sql="SELECT Upass from tb_nuser where num_ot=".$num_ot;
		...
		public function getSEVs($num_ot){
			$sql="SELECT p.*,po.*
				  from sev_person p
					left outer join sev_position po on (p.position_id = po.position_id)
		...
		public function getNuser($num_ot){
			$sql="SELECT * FROM tb_nuser WHERE num_ot=".$num_ot;
		...
		public function getPerson($num_ot){
			$sql="SELECT * FROM tb_person WHERE num_ot=".$num_ot;
			$result=$this->db->query($sql)->result();
			$numrow=$result->num_rows();
